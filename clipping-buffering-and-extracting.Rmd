...
::: callout-warning
**Warning:** [DIDN'T FINISH PICKING FINAL 5 MOORINGS BECAUSE DIDN'T KNOW WHAT CRITERIA TO GO BY...]{.underline} (added section for **Near (Analysis)** tool so would know how to calculate distance in future scenarios, **BUT ignoring results** for the purpose of this activity)!
:::

Open the **Near (Analysis)** tool from search bar:

```{r}
# Input Features: Mooring_CRCHbuffer50m
# Near Features: NOAA_clipSTEER_habitatCRCH
...
-   In the [Contents]{.underline} side pane under '[Map]{.underline}', select the `Mooring_CRCHbuffer50m` shapefile. **Data** top ribbon \> open **Attribute Table**.
...
-   **Table** top ribbon \> **Sort** drop-down menu \> **Custom Sort** \> Field: `NEAR_DIST` \| Sort: Descending.
...
**Data** top ribbon \> **Layer From Selection** \> rename new layer as `Mooring_CRCHbuffer50m_shallower30ft`.

::: callout-warning
**Warning:** DIDN'T FINISH PICKING FINAL 5 MOORINGS, [**BUT**]{.underline} since the mooring points in `Mooring_CRCHbuffer50m_shallower30ft` are clustered into 5 separate areas within the STEER boundary, would probably pick the 1 point from each cluster that has highest `NEAR_DIST` value (aka furthest from a coral reef/colonized hardbottom feature)!
:::
